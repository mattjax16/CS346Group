...
Na_out = zeros(1, number_iterations); %initialize the Na ion outside array
Na_out(1) = 150; %concentration of ions outside of the nueorn mM/L
Na_in = zeros(1, number_iterations); %initialize the NA ion in array
Na_in(1) = 15; %concentration of sodium loutside of nueron mM/L
K_out = zeros(1, number_iterations); %initialize K ion array outside
K_out(1) = 5.5; %concentration of potassium outside the nueron mM/L 
K_in = zeros(1, number_iterations); %initialize K ion array inside
K_in(1) = 150; %concentration of potassium outside mM/L
...
change_in_Na = @(voltage, n, m, h) (sodium_channel_current(voltage, n, m, h) ...
    );

change_in_K = @(voltage, n, m, h) (potassium_channel_current(voltage, n, m, h) ...
*(voltage < Na_K_threshold)*(deploroization == 1)*(hyperpolorization == 0)...
+ leakage_current(voltage, n, m, h));



%Pump Function
ATPase_pump = @(voltage, n, m, h) (leakage_current(V(1),n(1),n(1),n(1))...
    * -1 * (voltage < V_init));
...
for i = 2:number_iterations
    ...
    delta_na = change_in_Na(V(i), n(i), m(i), h(i));
    
    Na_in(i) = Na_in(i-1) - delta_na;
    Na_out(i) = Na_out(i-1) + delta_na;
    if i > 500 && i <510
        display(Na_out(i));
    end
    %K(i) = K(i-1) + (change_in_K(V(i), n(i), m(i), h(i))*K(i-1));
