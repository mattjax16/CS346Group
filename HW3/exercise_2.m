...
%set the pump current opisite of leakage to offset it
I_p = -(leakage_current(V(1), n(1), m(1), h(1)));
       
for i = 2:number_iterations
    time_array(i) = (i - 1) * delta_time;
    
    k1 = delta_time * dvdt(V(i - 1), n(i - 1), m(i - 1), h(i - 1));
    ...
    k2 = delta_time * dvdt(V(i - 1) + (0.5 * k1), n(i - 1) + ...
    ...
    k3 = delta_time * dvdt(V(i - 1) + (0.5 * k2), n(i - 1) + ...
    ...
    k4 = delta_time * dvdt(V(i - 1) + k3, n(i - 1) + n3, m(i - 1) + m3, ...
        h(i - 1) + h3);
    ...
    V(i) = V(i - 1) + 1 / 6 * (k1 + 2 * k2 + 2 * k3 + k4) - I_p;
