...
% simulation
numMonths = 120; %(int) This is the number of months to tun the sim
...
tuna_population = 30;  %(float, variable Y in equation)
    
human_population = 10; %(float, variable H in equation)



human_birth_fraction_tuna = 0.04;%(b_2 should probably be a float less than
                            % 1 ,this is because represnts number of
                            % humans produced from one shark)

human_birth_fraction_shark = 0.1;%(b_1 should probably be a float less than
                            % 1 ,this is because represnts number of
                            % humans produced from one shark)

human_death_proportionality_constant = 0.02;%(K_H)

fishing_rate_tuna = 0.03;%(float between 0 and 1, reresents the 
                          % rate-constant K_YH)
                          
fishing_rate_shark = 0.01;%(float between 0 and 1, reresents the 
                          % rate-constant K_PH)
...
%           (dY/dt) = K_Y(Y) - K_PY(YP) - K_YH(YH)
    ...
    max_pop = max(max(max(sharkArray,tunaArray)),max(humanArray));
    
    [vect_tuna,vect_shark,vect_human]=meshgrid(0:50:max_pop,...
        0:50:max_pop,0:50:max_pop);
%     
%     % Tuna 
%     tuna_births = tuna_birth_fraction*tuna_population; % K_Y(Y)
%     tuna_deaths = tuna_population*((tuna_death_proportionality_constant * ...
...
%     human_deaths = human_death_proportionality_constant*human_population;
    ...
    U = (tuna_birth_fraction.*vect_tuna)-(vect_tuna.*...
        ((tuna_death_proportionality_constant.* vect_shark) + ...
        (fishing_rate_tuna.* vect_human)));

    V = (shark_birth_fraction.* tuna_death_proportionality_constant...
         .* vect_tuna .* vect_shark) - (fishing_rate_shark .* vect_shark ...
         .*vect_human);
    
    W = ((human_birth_fraction_shark .* shark_deaths + ...
        human_birth_fraction_tuna .* fishing_rate_tuna .* vect_tuna)...
        .* vect_human) - (human_death_proportionality_constant .* ...
        vect_human);

    L=sqrt(U.^2+V.^2+W.^2);
    quiver3(vect_tuna,vect_shark,vect_human,U./L,V./L,W./L,1,'k')
    hold on
    scatter3(tunaArray,sharkArray,humanArray)
    xlabel("Tuna(Y)")
    ylabel("Sharks(P)")
    zlabel("Humans(H)")
...
tuna_death_proportionality_constant = tuna_death_proportionality_constant+0.005;
shark_population = 20; %(float, variable P in equation)   
tuna_population = 30;  %(float, variable Y in equation)
...
%make the pair plots if true
if show_pop_pair_plots
    populationNames = ["Tuna(Y)","Sharks(P)","Humans(H)"];
    figure
    populationArrays = ones(3,numIterations) ;
    populationArrays(1,:) = tunaArray;
    populationArrays(2,:) = sharkArray;
    populationArrays(3,:) = humanArray;
    num_ax = 1;
    for graphX = 1:3
        for graphY  = 1:3
            subplot(3,3,num_ax)
            scatter(populationArrays(graphX,:),populationArrays(graphY,:))
            num_ax = num_ax+1;
            
            %set axisis
            if graphY == 1
                ylabel(populationNames(graphX))
            end
            if graphX == 3
                xlabel(populationNames(graphY))
            end
        end
    end
    ...
    % make title
    sgtitle(["Predator Prey Model of Sharks Tuna and "+...
        "Humans over " + {numMonths} + " Months","Pair Plots","","dt= "...
        + {dt} + "   K_H= " + {human_death_proportionality_constant} + ...
        "   K_P_H= " + ...
        {fishing_rate_shark} + "   K_Y_H= " + {fishing_rate_tuna} + ...
        "   K_Y= " + {tuna_birth_fraction} + "   K_P_Y= " + ...
        {tuna_death_proportionality_constant},"b_0= " + ...
